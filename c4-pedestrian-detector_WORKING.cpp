...
int main()
{
	//std::cout << video_file_name << " <video_file>" << std::endl << std::endl;
	...
	cv::VideoCapture capture(userSource);
	cv::Mat src;
    ...
    while( key != 27 )
    {
        ...
        for(size_t i = 0; i < results.size(); i++)
        {
			...
			cv::Point centerPoint((results[i].right - results[i].left) / 2 + results[i].left, (results[i].bottom - results[i].top) / 2 + results[i].top);
			cout << centerPoint.x << ", " << centerPoint.y << endl;
			//ofs << centerPoint.x << ", " << centerPoint.y << endl;
			//cv::circle(src, centerPoint, 5, cv::Scalar(0,0,255),2,8,0 );

			cv::line(src, cv::Point(centerPoint.x - 10, centerPoint.y), cv::Point(centerPoint.x + 10, centerPoint.y), cv::Scalar(0, 0, 255), 2);
			cv::line(src, cv::Point(centerPoint.x, centerPoint.y - 10), cv::Point(centerPoint.x, centerPoint.y + 10), cv::Scalar(0, 0, 255), 2);
        }
		...
		{
			wait_time = 0;
			cout << "0 : waits to process next frame until a key pressed" << endl;
		}			

		if (key == 49)
		{
			wait_time = 1;
			cout << "1 : doesn't wait to process next frame" << endl;
		}			

		if (key == 50)
		{
			fx = 0.5;
			cout << "2 : resize frames 1 / 2" << endl;
		}			

		if (key == 51)
		{
			fx = 1;
			cout << "3 : don't resize frames" << endl;
		}			

		if (key == 52)
		{
			fx = 0.25;
			cout << "4 : resize frames 1/4" << endl;
		}
